...
6. **User Interaction for Ambiguity Resolution**
   ...
   - Use `interactive_feedback` tool to present questions systematically:
...
7. **Generate OpenSpec Proposal**
    - **AUTO-EXECUTE**: Directly run `/opsx:ff <requirement-description>` as soon as constraints are finalized (i.e., immediately after Step 5 if no questions, or after User answers in Step 6). Do not pause to summarize.
...
- Use `interactive_feedback` for ANY ambiguity—do not assume or guess.
