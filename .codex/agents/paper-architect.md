...
- Treat `#### Theory hook` as a method-facing theorem-design placeholder that states the intended theory target and theorem roles. The final two theorems will be written by `theorem-architect` to serve this method section rather than revisit repository code details.
...
Inside `#### Theory hook`, describe the intended theorem pair, the central quantities, the target observable phenomena, and how the pair serves the method section, but do not fabricate finished theorem statements or repository-specific proof obligations.
